...
export function MatchCard({ match, saved, onSave, members = [] }: { match: Match; saved?: Prediction; onSave: (p: Prediction) => void; members?: Member[] }) {
  ...
  return <article className="min-w-[310px] snap-start rounded-[24px] border border-ink/10 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-card sm:min-w-[350px]">
    <div className="flex items-center justify-between"><span className={`rounded-full px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.13em] ${windowState.isOpen ? "bg-lime text-forest" : "bg-forest/7 text-forest"}`}>{windowState.isOpen ? "Predictions open" : match.stage === "GROUP" ? match.group : match.stage.replace(/_/g, ' ')}</span><span className="flex items-center gap-1.5 text-[11px] font-bold text-ink/45"><Clock3 size={13}/>{mounted ? formatKickoffToIST(date) : ""}</span></div>
    <div className="mt-6 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><Team flag={match.home.flag} name={match.home.name}/><div className="flex flex-col items-center gap-2"><div className="flex items-center gap-2"><Score value={home} set={setHome} label={`${match.home.name} score`} disabled={!windowState.isOpen || !isEditing}/><span className="font-display text-lg font-bold text-ink/25">:</span><Score value={away} set={setAway} label={`${match.away.name} score`} disabled={!windowState.isOpen || !isEditing}/></div></div><Team flag={match.away.flag} name={match.away.name} right/></div>
    ...
    <div className="mt-4 flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3 text-sm text-ink/60">
        <label className={`inline-flex items-center gap-2 rounded-full border border-ink/10 bg-cream px-3 py-2 ${(!windowState.isOpen || !isEditing) ? 'opacity-50 pointer-events-none' : ''}`}>
          <input type="checkbox" checked={sameForAll} disabled={!windowState.isOpen || !isEditing} onChange={(event) => setSameForAll(event.target.checked)} className="h-4 w-4 rounded border-ink/20 text-forest focus:ring-forest/60" />
          <span>Apply this pick to all leagues</span>
        </label>
        <span className="text-xs uppercase tracking-[.16em] leading-tight flex-1">{windowState.isOpen ? "Same pick will save across leagues" : windowState.state === "scheduled" ? "Locked until 24h before kickoff" : "Predictions are locked"}</span>
      </div>
    </div>
    <div className="mt-6 flex items-center justify-between border-t border-ink/8 pt-4">
      ...
      <button 
        onClick={submit} 
        disabled={!windowState.isOpen || (isEditing && (home === "" || away === "" || (needsPenaltyWinner && !penaltyWinnerId)))} 
        className={`flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-[11px] font-extrabold transition ${flash ? "bg-lime text-forest" : windowState.isOpen ? (isEditing ? "bg-ink text-white hover:bg-forest" : "bg-cream text-ink hover:bg-ink/10") : "bg-ink/7 text-ink/45"} disabled:cursor-not-allowed`}
      >
        {flash ? <Check size={14}/> : !windowState.isOpen ? <LockKeyhole size={14}/> : null} 
        {flash ? "Locked in" : !windowState.isOpen ? (windowState.state === "scheduled" ? `Opens in ${formatTimeUntil(windowState.opensAt, now)}` : "Predictions closed") : isEditing ? (saved ? "Update pick" : "Save pick") : "Edit pick"}
      </button>
    </div>
  </article>;
}
